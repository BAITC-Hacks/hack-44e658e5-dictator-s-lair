...
function normalize(raw){
  ...
  const invalid=field=>{throw new Error(`Сервер вернул некорректные данные совещания (${field}). Повторите обработку записи.`)};
...
async function readApiJson(response){try{return await response.json()}catch{throw new Error('Сервер вернул некорректный JSON. Повторите обработку.')}}
async function processMeeting(file){
  ...
  try{
    ...
    if(!queued||typeof queued.job_id!=='string'||!queued.job_id)throw new Error('Сервер не вернул идентификатор обработки (job_id).');
    for(let attempt=0;attempt<360;attempt++){
      ...
      if(job?.status==='failed')throw new Error(job.error||'Сервер не смог обработать аудиозапись.');
      if(job?.status==='completed')return normalize(job.result);
      throw new Error('Сервер вернул неизвестный статус обработки.');
    }
    ...
    if(error instanceof TypeError)throw new Error('Нет соединения с сервером. Проверьте, что API запущен на 127.0.0.1:8000.');
    throw error;
  }finally{clearTimeout(timer)}
}
function show(id){['uploadView','processingView','resultView'].forEach(x=>$(x).hidden=x!==id)}
function start(file){if(workspaceBusy()||!mayLeaveMeeting())return;releaseAudio();state.processing=true;state.data=null;state.historyId=null;state.historyRecord=null;state.historySaved=false;state.historyUnsaved=false;state.audioFile=file;syncWorkspaceNavigation();$('errorBox').hidden=true;$('exportTopBtn').hidden=true;$('processingFile').textContent=file?.name||'Запись совещания';$('processingHint').style.color='';show('processingView');setProcessingStage('stt');return processMeeting(file).then(async data=>{state.data=data;render();show('resultView');$('exportTopBtn').hidden=false;$('pageTitle').textContent='Протокол совещания';await saveCompletedMeeting(file,data)}).catch(error=>{console.error(error);showProcessingError(error.message||'Не удалось обработать эту запись')}).finally(()=>{state.processing=false;syncWorkspaceNavigation()})}
...
function syncWorkspaceNavigation(){
  ...
  document.querySelectorAll('input[data-field],textarea[data-field]').forEach(input=>input.disabled=state.restoring);
...
async function openHistory(id){
  ...
  try{
    ...
    $('errorBox').hidden=true;render();show('resultView');$('exportTopBtn').hidden=false;$('pageTitle').textContent='Протокол совещания';
    $('nowPlaying').textContent='Нажмите «Проверить в аудио» у поручения или выберите таймкод в транскрипте.';
...
function render(){
  ...
  $('meetingDate').textContent=m.date||'Дата не указана';
  $('meetingDuration').textContent=`Длительность: ${fmtTime(m.duration_seconds)}`;
  $('meetingLanguage').textContent=({ru:'Русский',kk:'Казахский',en:'Английский'})[m.language]||m.language||'Язык не указан';
  $('summaryText').textContent=s.executive_summary||'';
  $('topicList').innerHTML=(s.key_topics||[]).map(x=>`<span class="topic">${esc(x)}</span>`).join('');
...
function taskHtml(x,i){
  return `<article class="task-card">
    <div class="task-header"><span class="task-number" aria-hidden="true">${String(i+1).padStart(2,'0')}</span><div class="task-title"><textarea rows="2" class="task-title-input" aria-label="Поручение ${i+1}" data-field="task" data-index="${i}">${esc(x.task)}</textarea></div></div>
    <div class="task-fields"><label class="field-chip editable"><b>Ответственный</b><input data-field="assignee" data-index="${i}" value="${esc(x.assignee??x.assignee_speaker_id??'')}" placeholder="Не определён" /></label><label class="field-chip editable"><b>Срок исполнения</b><input data-field="deadline" data-index="${i}" value="${esc(x.deadline??'')}" placeholder="Срок не указан" /></label></div>
    <div class="task-footer"><div class="source-quote">«${esc(x.source_quote||x.task)}»</div><div class="task-controls"><button class="evidence-btn" data-evidence="${x.timestamp_start}" data-index="${i}">▶ Проверить в аудио · ${fmtTime(x.timestamp_start)}</button><button class="save-btn" data-save="${i}">Сохранить</button></div><div class="task-context"><span>Говорит: ${speakerLabel(x.speaker_name??x.speaker??'—')}</span><span>Первоисточник: ${fmtTime(x.timestamp_start)}–${fmtTime(x.timestamp_end)}</span><span>Уверенность модели: ${Math.round((x.confidence||0)*100)}%</span></div></div>
  </article>`
}
function seek(time,index){const p=$('audioPlayer');if(p.src){p.currentTime=time;p.play().catch(()=>{})}const segment=state.data.transcript.findIndex(x=>Number(x.start)<=time&&Number(x.end)>=time);const active=segment>=0?segment:index;$('nowPlaying').textContent=`Первоисточник · ${fmtTime(time)} — ${state.data.transcript[active]?.text||'фрагмент записи'}`;document.querySelectorAll('.transcript-line').forEach(x=>x.classList.toggle('active',x.dataset.index==active))}
